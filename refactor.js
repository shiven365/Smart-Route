...
const importsStr = splitLines.slice(0, startIdx).join('\n');
const funcsStr = splitLines.slice(startIdx, reactCompIndex).join('\n');
const compStr = splitLines.slice(reactCompIndex).join('\n');
...
// Extract graphUtils and algorithms from the string using regex/manual search
// Actually, it's easier if I just dump the whole utilities into those files from original string
